...
export class Router {
  private routes: Array<{
    ...
    pattern: URLPattern;
    rawPath: string;
    handler: (req: Request, env: any, ctx: ExecutionContext, params: Record<string, string>) => Promise<Response>;
  }> = [];
  ...
  registerController(ctrlClass: any) {
    ...
    const routes: RouteDefinition[] = Reflect.getMetadata(ROUTES_KEY, ctrlClass) || [];
    const ctrlMiddlewares = Reflect.getMetadata(MIDDLEWARES_KEY, ctrlClass) || [];
    ...
    for (const route of routes) {
      ...
      const routeMiddlewares = Reflect.getMetadata(`${MIDDLEWARES_KEY}:${route.handlerName}`, ctrlClass) || [];
      ...
      this.routes.push({
        ...
        handler: async (req, env, ctx, pathParams) => {
          // Run controller-level + route-level middlewares
          for (const mw of [...ctrlMiddlewares, ...routeMiddlewares]) {
            const result = await mw(req, env);
            if (result instanceof Response) return result;
          }

          const paramsMeta: ParamMetadata[] =
            Reflect.getMetadata(`${PARAMS_KEY}:${route.handlerName}`, ctrlClass) || [];

          const args = await resolveHandlerArgs(req, env, pathParams, paramsMeta);
  ...
  async resolve(request: Request, env: any, ctx: ExecutionContext): Promise<Response> {
      ...
      if (match) {
        ...
        try {
          ...
          const response = err instanceof HttpException
            ? err.toResponse()
            : jsonResponse({ error: err.message || 'Internal Server Error' }, 500);
          return applyCorsHeaders(request, response);
        }
      }
    }

    return applyCorsHeaders(request, jsonResponse({ error: 'Not Found', path: url.pathname }, 404));
  }
}
...
async function resolveHandlerArgs(
  ...
  for (const meta of paramsMeta) {
    ...
    switch (meta.type) {
      ...
      case 'header':
        value = meta.key ? req.headers.get(meta.key) : Object.fromEntries(req.headers);
        break;
    }
...
function applyCorsHeaders(request: Request, response: Response): Response {
  const corsHeaders = (request as any).__corsHeaders as Record<string, string> | undefined;
